...
function osiris_builtin_map(fn, ...iterables) {
...
function osiris_builtin_enumerate(iterable, start=0) {
...
function osiris_builtin_divmod(x, y) {
  if (y === 0) {
...
function osiris_builtin_round(number, precision = 0) {
  const factor = Math.pow(10, precision);
  return (Math.round(number * factor) / factor);
}
function osiris_builtin_zip(...arrays) {
...
function osiris_builtin_sorted(iterable, { key = null, reverse = false } = {}) {
  let arr = [...iterable];
  arr.sort((a, b) => {
...
function osiris_builtin_max(iterable, key = x => x) {
...
function osiris_builtin_sum(iterable, start = 0) {
  let total = start;
...
function osiris_builtin_range(start, stop, step = 1) {
  if (stop === undefined) {
    stop = start;
    start = 0;
  }
...
function osiris_builtin_len(input) {
...
function osiris_builtin_extend(arr, iterable) {
 ...
 function convertPythonOperand(value) {
    if (typeof value === 'boolean') {
        return value ? 1 : 0;
    }
    return value;
...
function osiris_builtin_addition(a, b) {
...
function osiris_builtin_subtraction(a, b) {
    a = convertPythonOperand(a);
    b = convertPythonOperand(b);
...
function osiris_builtin_multiplication(a, b) {
    a = convertPythonOperand(a);
    b = convertPythonOperand(b);
...
function osiris_builtin_get(obj, key, defaultValue = null) {
...
function osiris_builtin_filter(predicate, iterable) {
...
function osiris_builtin_int(value, base = 10) {
    if (typeof value === 'boolean') {
        return value ? 1 : 0;
    }
