...
import LogoParser from './grammar/LogoParser.js';
import LogoLexer from './grammar/LogoLexer.js';
import LogoCodeGenerator from './transpilerLogoJs.js';
...
function parseLogo(input) {
    ...
    const lexer = new LogoLexer(inputStream);
...
/**
 * Translates Logo code to JavaScript.
 ...
 */
export function translateLogo(input,runOnBrowser) {
    try {
...
class Osiris
{
    ...
    passCode(code) {
        this.code = code;
        this.transpiledCode = translateLogo(this.code, false);
    ...
    runCode(appendToTerminal, timeout) {
      try {
        ...
        let insideCode = translateLogo(this.code, true).code;
